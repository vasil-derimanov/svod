...
def run_batch_test(video_files: list, old_script: str, new_script: str, max_videos: int = 10):
    ...
    for video_path in video_files[:max_videos]:
        tested_count += 1
        print(
            f"📹 Video {tested_count}/{min(max_videos, len(video_files))}: {os.path.basename(video_path)}"
        )

        # Test old version
        old_success, old_orientation, old_confidence, old_time, old_error, old_recommendation = (
            test_video_with_detector(old_script, video_path)
        )

        # Test new version
        new_success, new_orientation, new_confidence, new_time, new_error, new_recommendation = (
            test_video_with_detector(new_script, video_path)
        )
        ...
        time_icon = (
            "⚡"
            if time_diff < 0 and old_success and new_success
            else "🐌" if time_diff > 0 and old_success and new_success else "⏱️"
        )
    ...
    time_improvements = sum(
        1 for r in results if r["time_diff"] < 0 and r["old_success"] and r["new_success"]
    )
    ...
    if successful_tests > 0:
        avg_time_diff = (
            sum(r["time_diff"] for r in results if r["old_success"] and r["new_success"])
            / successful_tests
        )
        print(".1f")
...
def main():
    ...
    parser.add_argument(
        "--max-videos", type=int, default=5, help="Maximum number of videos to test"
    )
