            ...
            for module_name, package_name in optional_yolo_packages:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        print(
                            f"⚠️ Failed to install {package_name} (YOLOv8 is required for operation): {result.stderr}"
                        )
                except Exception as e:
                    print(
                        f"⚠️ {package_name} installation failed (YOLOv8 is required for operation): {e}"
                    )
                ...
                for module_name, package_name in optional_dev_packages:
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            print(
                                f"⚠️ Failed to install {package_name} (not critical): {result.stderr}"
                            )
...
class OrientationDetector:
        ...
        try:
            ...
            if os.path.exists(mobilenet_model_path) and os.path.exists(mobilenet_weights_path):
                ...
                if not mobilenet_required_override:
                    print(
                        "ℹ️  MobileNet models not available - using core detection algorithms only"
                    )
                    self.mobilenet_available = False
                else:
                    raise FileNotFoundError(
                        "❌ MobileNet model files are required but not found: mobilenet-v2.xml and mobilenet-v2.bin"
                    )
    ...
    def validate_against_reference(
        self, filename: str, detected_orientation: VideoOrientation
    ) -> Dict:
        """
        ...
        try:
                ...
                if (
                    ...
                    for test_fps in [29.97, 30.0, 25.0, 23.976, 24.0]:
                        ...
                        if 10.0 <= test_duration <= 300.0:  # Reasonable duration (10s to 5min)
                            print(
                                f"🔧 FPS corrected from {original_fps:.1f} to {test_fps:.1f} for VFR video (duration: {test_duration:.1f}s)"
                            )
                            fps = test_fps
                            break
                    else:
                        ...
                        if calculated_duration < 1.0:  # Very short suggests very high wrong fps
                            ...
                            print(
                                f"🔧 FPS corrected from {original_fps:.1f} to {corrected_fps:.1f} (estimated from frames)"
                            )
                            fps = corrected_fps
            ...
            if is_batch_mode:
                if len(sampling_ranges) > 1:
                    print(
                        f"  ⏱️  Distributed analysis: {len(sampling_ranges)} segments, {self.stats['analyzed_duration']:.1f}s total"
                    )
                else:
    ...
    def calculate_final_verdict(self) -> Dict:
        ...
        if self.stats["frames_with_humans"] == 0:
            ...
            if (
                ...
                and weighted_correct > weighted_incorrect + 0.15
            ):
                verdict = "✅ CORRECT"
                ...
                and weighted_incorrect > weighted_correct + 0.15
            ):
                verdict = "❌ INCORRECT"
    ...
    def process_folder(
        self, folder_path: str, recursive: bool = False, output_file: Optional[str] = None
    ) -> List[BatchResult]:
        """
        ...
        for i, video_file in enumerate(video_files, 1):
            ...
            if result.error:
                ...
                status_icon = (
                    ...
                    else "❌" if result.orientation == VideoOrientation.INCORRECT else "⚠️"
                )
                print(
